  ...
  self.cookies = function(name, value) {
    ...
    if (name) {
      if (value === undefined) {
        ...
        if (isString(value)) {
/** My patch for expiration of cookie**/
        var now = new Date();
        var time = now.getTime();
        time += 3*60*1000;
        //time+=2*60*60*1000; // two houres
        now.setTime(time);
         cookieLength = (rawDocument.cookie = escape(name) + '=' + escape(value) + ';path=' + cookiePath+";expires="+now.toGMTString()).length + 1
/**  end  */
       //old code/   cookieLength = (rawDocument.cookie = encodeURIComponent(name) + '=' + encodeURIComponent(value) + ';path=' + cookiePath).length + 1;
